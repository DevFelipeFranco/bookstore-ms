...
public record PersonalInfo(
        ...
        return Stream.ofNullable(name)
                ...
                .orElseThrow(() -> new IllegalArgumentException(
                        "%s must be between %d and %d characters and cannot be empty. Provided: [%s]".formatted(
                                fieldName, MIN_NAME_LENGTH, MAX_NAME_LENGTH, name)));
    }
     ...
     * @throws IllegalArgumentException si la validación falla
     */
    private static final String PHONE_ERROR_MSG = "Invalid phone number format: %s. Must contain only numbers and symbols +()-, and be between 7-15 characters";
    ...
    private static String validateAndNormalizePhone(String phone) {
        return Stream.ofNullable(phone)
                ...
                .findFirst() // Convertir Stream a Optional
                .orElseThrow(() -> new IllegalArgumentException(PHONE_ERROR_MSG.formatted(phone)));
    }
